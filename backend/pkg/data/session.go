...
func (s Session) Add(db *sql.DB, session *Session) error {
	query := `insert into sessions (user_id,
	 email, expiry, token, activate_code, user_ip, type, post_suspend_expiry)
	  values (?,?,?,?,?,?,?,?)`
	_, err := db.Exec(query,
		session.UserId,
		session.Email,
...
func (s Session) GetByToken(db *sql.DB, token string) (*Session, error) {
	query := "SELECT user_id, email, expiry, token, activate_code, user_ip, type, post_suspend_expiry FROM sessions WHERE token = ?"
	var session Session
